...
// Phase encoding for GTKWave
reg [0:4] phase_enc;
always @(*) begin
    casez (phase)
        20'b1???????????????????: phase_enc = 5'd0;   // PCP5
        20'b01??????????????????: phase_enc = 5'd1;   // PREP1
        20'b001?????????????????: phase_enc = 5'd2;   // PREP2
        20'b0001????????????????: phase_enc = 5'd3;   // PREP3
        20'b00001???????????????: phase_enc = 5'd4;   // EX1
        20'b000001??????????????: phase_enc = 5'd5;   // EX2
        20'b0000001?????????????: phase_enc = 5'd6;   // EX3
        default:                  phase_enc = 5'd31;
    endcase
end
...
always @(posedge clock) begin
    if (reset) begin : rr_reset
        ...
        RR[R] <= A;   // RR[r] ← A, using R register directly
end
...
always @(*) begin
    ...
    case (1'b1)
        ...
        // ------------------------------------------------------------------
        // PCP5: assert ENDE — P+4=0x98, fetches M[0x98] for PREP1
        // LCFI already in C/D/O from reset, no need to execute phases
        // ------------------------------------------------------------------
        phase[0]: begin
            ende = 1'b1;
        end
        ...
        phase[4]: begin
            case (O)
                ...
                OP_LW: begin
                    C_load   = 1'b1;          // C ← M[EA]
                    A_load   = 1'b1;
                    A_in     = C_mux;         // A ← C_mux (transparent)
                    bus_addr = {Q, 2'b00};
                end
        ...
        phase[5]: begin
            case (O)
                OP_LCFI: ende    = 1'b1;
                OP_LW: begin
                    rr_write = 1'b1;          // RR[R] ← A
                    P_load   = 1'b1;
                    P_in     = {Q, 2'b00};    // P[15:31] ← Q (restore next instr addr)
                    bus_addr = {Q, 2'b00};
                end
                default: bus_addr = {Q, 2'b00};
            endcase
